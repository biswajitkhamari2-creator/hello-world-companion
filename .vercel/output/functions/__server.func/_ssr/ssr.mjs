...
async function getServerEntry() {
	if (!serverEntryPromise) serverEntryPromise = import("./server-vthkrsu9.mjs").then((m) => m.default ?? m);
	return serverEntryPromise;
}
function isApiRequest(request) {
	return new URL(request.url).pathname.startsWith("/api/");
}
function renderApiError(status = 500, message = "Something went wrong. Please retry.") {
...
async function normalizeCatastrophicSsrResponse(request, response) {
	...
	if (isApiRequest(request)) return renderApiError();
